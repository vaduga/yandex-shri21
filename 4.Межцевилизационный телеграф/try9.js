...
let prep = Object.keys(mySet).map(el=> [el,mySet[el]])
console.log(prep) 
  
  
  let final = prep.map((el)=>  {
      let res = ''
      let amount2 = amount
    while (amount2 >= parseInt(el[0])) {
    
      res+=el[1][0]
      amount2 -= parseInt(el[0])
    }
    return res
  }
  ...
  return final.map(el=> parseInt(el)).sort((a,b)=> b-a )[0].toString()
  
  }


let r = spend(0,[9,11,1,12,5,8,9,10,6]);
console.log(r)
